...
exports.getMapData = function(params, callback) {
    let query_string = "select measuring_date, wind_turbine_serial_id_fk, variable_id_fk, wtu.nominal_power, "+
       "COALESCE(((COALESCE(measuring_value_interval_1,0)+" +
			...
			"IF(measuring_value_interval_144 is null, 0, 1))) , 0) AS value \
        from lvs_measuring_data.wind_turbine_unit_measuring_data_daily wtumdd \
		LEFT OUTER JOIN lvs_server_database.wind_turbine_units wtu \
			ON wtumdd.wind_turbine_serial_id_fk = wtu.wind_turbine_serial_id \
    ...
    lvs_db.sequelize.query(query_string, {
      replacements:{
        turbines: params.turbine_id,
        variable: [1372,569,614] ,
        date: date ,
        type:2
      },
